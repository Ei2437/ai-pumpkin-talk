...
def get_speakers():
    r = requests.get(f"{BASE}/speakers", timeout=5)
    r.raise_for_status()
    return r.json()

def create_audio_query(text, speaker):
    r = requests.post(f"{BASE}/audio_query", params={"text": text, "speaker": speaker}, timeout=5)
    r.raise_for_status()
    return r.json()

def synthesis(audio_query, speaker, out_file):
    r = requests.post(f"{BASE}/synthesis", params={"speaker": speaker},
                      headers={"Content-Type": "application/json"},
                      json=audio_query, timeout=30)
    r.raise_for_status()
...
def main():
    try:
        ...
        # 最初の speaker の最初の style の id を採る（互換性のため）
        try:
