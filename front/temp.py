...
def main():
    ...
    while True:
        ...
        # 状態管理
        if state == "normal":
            frame, dx, dy = handle_a_key_video(videos["normal"], a_pressed, t, seed=1)
            draw_video(screen, frame, dx, dy)
        elif state == "full3":
            frame, dx, dy = handle_a_key_video(videos["full3"], a_pressed, t, seed=3)
            draw_video(screen, frame, dx, dy)
        elif state == "full6":
            frame, dx, dy = handle_a_key_video(videos["full6"], a_pressed, t, seed=5)
            draw_video(screen, frame, dx, dy)
        elif state == "full2":
